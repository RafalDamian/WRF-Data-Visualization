...
let µ = function() {
    ...
    let τ = 2 * Math.PI;
    let H = 0.0000360;  // 0.0000360°φ ~= 4m
    let DEFAULT_CONFIG = "current/wind/surface/level/orthographic";
    let TOPOLOGY = isMobile() ? "/data/earth-topo-mobile.json?v2" : "/data/my-topojson.json?v2";
    ...
    function loadJson2(type) {
        const script = document.querySelector("script[data-name='micro']")
        const wind = JSON.parse(script.getAttribute('wind'))
        const promise = new Promise(resolve => wind, reject => console.log('Something went wrong while reading ', type))
        //console.log(promise)
        return promise
    }
